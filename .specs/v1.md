...
- Standings table should flow top-bottom then left-right
- Standings should contain starting grid
- Standings should show position diff with up and down triangular arrows side, should always show both side by side next to the position number, each either grey or current theme accent color
    - Championship: from immediate previous race to current latest
    - Latest race: from starting grid to final position
- Previous and Current race session cards should show cards of podium drivers