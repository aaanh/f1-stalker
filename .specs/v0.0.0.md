...
- Icons: [Lucide](https://lucide.dev/) SVGs vendored under `src/assets/icons/`, rendered via Iced `svg` (`src/ui/icons.rs`)
- Database: SQLite
...
All in-app glyphs use **Lucide** stroke icons. SVG files are copied into the repo (not fetched at runtime) and tinted with the active theme palette.

| Layer | Location | Role |
| ----- | -------- | ---- |
| Assets | `src/assets/icons/*.svg` | Lucide-compatible SVGs (`stroke="currentColor"`, 24×24 viewBox) |
| Registry | `src/ui/icons.rs` | `Icon` enum, `icon()`, `icon_label()`, `section_heading()` |
| Consumers | `src/ui/*` | Section headings, buttons, boot steps, pin controls, standings arrows |

**Rendering rules**

- Use `icon(Icon::…, size, color)` or `icon_label()` / `section_heading()`; do not use Unicode symbols or ad-hoc text glyphs for UI chrome.
- Colour comes from the theme (`text_color()`, `accent()`, `muted()`, `live()`, etc.) via Iced `svg::Style { color: Some(…) }`.
- Section titles pair a 16px icon with a 15px label (`section_heading`).
- Buttons that need a glyph use `secondary_button_icon`, `action_button_icon`, or `danger_button_group` with an `Icon` variant.

**Icon inventory (v0 baseline)**

| `Icon` variant | Lucide name | Typical use |
| -------------- | ----------- | ----------- |
| `Dashboard` | `layout-dashboard` | Shell nav |
| `Settings` | `settings` | Shell nav, settings section |
| `Trophy` | `trophy` | Championship charts, quali grid, standings |
| `Users` | `users` | Pinned drivers, rivalry section, driver picker |
| `UserPlus` | `user-plus` | Add driver pin |
| `Pin` / `PinOff` | `pin` / `pin-off` | Pin driver, unpin, picker header |
| `ChevronLeft` / `ChevronRight` | `chevron-left` / `chevron-right` | Reorder pins, sort direction |
| `ChevronUp` / `ChevronDown` | `chevron-up` / `chevron-down` | Standings position change (both shown; active direction uses accent) |
| `Refresh` | `refresh-cw` | Manual refresh, retry actions |
| `Search` | `search` | Driver picker filter |
| `Check` | `check` | Boot step done, settings toggle on |
| `Circle` / `CircleFilled` | `circle` / `circle-dot` | Boot pending, weekend live indicator |
| `Loader` | `loader` | Boot step running |
| `Alert` | `circle-alert` | Boot failure, notifications section |
| `Help` | `circle-help` | Title bar About |
| `Close` / `Minus` / `Maximize` | `x` / `minus` / `maximize-2` | Custom title bar window controls |
| `Copy` | `copy` | Copy debug log |
| `Trash` / `Database` | `trash-2` / `database` | Cache / database settings |
| `FileText` | `file-text` | Debug log section |

Country flags and team logos remain **raster images** from OpenF1 URLs, not Lucide icons.
...
| **Real-time data**             | Live OpenF1 feed (paid, authenticated). **Out of scope** for v0.0.0.1.0.                                                                                |
| **Lucide icon**                | UI glyph from the Lucide set, vendored as SVG and tinted at render time via `src/ui/icons.rs`. Not used for flags, headshots, or team logos.      |
...
| **Sprint grid**         | N/A                                 | From Sprint Qualifying; **not shown in v0.0.0.1.0**                                            |
...
| --- | ----- | ---------- | -------- | ----- |
| US1 | fan | see the previous, current, and upcoming Grand Prix at a glance | I know where the season is without digging through a calendar | v0.0.0.1.0 |
| US2 | fan | see a countdown to the next session (FP, quali, race) | I know when the next on-track action starts | v0.0.0.1.0 |
| US3 | fan | see forecast and track-side weather side by side for each GP | I can compare predicted conditions with what was recorded at the circuit | v0.0.0.1.0 |
| US4 | fan | switch between Drivers' and Constructors' championship charts | I can follow both title fights across the season | v0.0.0.1.0 |
| US5 | fan | see quali grid position and gap to pole for my pinned drivers | I know how they start the race after qualifying | v0.0.0.1.0 |
| US6 | user | open the app and still see the last fetched data when offline | a bad connection does not leave me with a blank screen | v0.0.0.1.0 |
| US0 | user | download a portable build for my CPU architecture (arm64 / x86_64) | I can run the app without installing dependencies | post-v0.0.0.1.0 |
| USF1 | user | have a desktop, launcher, or Docker icon | I can open the app readily | post-v0.0.0.1.0 |
| USF2 | user | be guided through first-run setup (timezone, optional pins) | the dashboard works correctly without reading docs | v0.0.0.1.0 |
| USF3 | fan | see who leads or has won the Drivers' Championship | I get a clear season narrative at a glance (e.g. title fight or confirmed winner) | v0.0.0.1.0 |
| USF4 | fan | use a rival mode view with exactly two drivers | I can compare two drivers head-to-head across the season (e.g. Verstappen vs Hamilton) | post-v0.0.0.1.0 |
| USF5 | fan | pin drivers and see their championship progress on a chart | I can follow my favourite drivers through the season | v0.0.0.1.0 |
| US6.1 | user | choose a theme from a default set of constructor livery colours | the app matches my team preference without manual tuning | post-v0.0.0.1.0 |
| US6.2 | user | create a custom theme | the dashboard looks the way I want | post-v0.0.0.1.0 |
| US7 | user | have my configuration and settings saved between sessions | I do not repeat setup every time I open the app | v0.0.0.1.0 |
| US8 | user | receive desktop notifications about races or pinned drivers | I am reminded of upcoming sessions or results without keeping the window open | post-v0.0.0.1.0 |
| US9.1 | user | keep the app running in the menu bar, taskbar, or as a background service when I close the window | it stays available without occupying my desktop | post-v0.0.0.1.0 |
| US9.2 | user | restore the main window from the background tray/service | I can return to the dashboard quickly | post-v0.0.0.1.0 |
| US9.3 | user | quit the app completely instead of sending it to the background | I can free resources when I am done | post-v0.0.0.1.0 |
...
### Icons and chrome (Lucide)

- [ ] Section headings use `section_heading()` with a Lucide icon + title (e.g. Trophy for charts, Users for pins)
- [ ] Navigation, buttons, boot steps, and window controls use `Icon` variants from `src/ui/icons.rs`, not Unicode symbol fallbacks
- [ ] Icons inherit theme colours (`text`, `accent`, `muted`, `live`) at render time
- [ ] Raster assets (country flags, driver headshots, team logos) stay image-based; Lucide is for UI chrome only
...
| TTL elapsed (off-weekend: ~1h calendar; on-weekend: ~1m optional post-v0.0.0.1.0) | Mark section stale; refresh on next poll |
| Settings changed (timezone, pins, API key) | Recompute view models; refetch affected sections |

### Transitions out of scope for v0.0.0.1.0

Post-v0.0.0.1.0 stories (US8 notifications, US9.x tray/background) add states **Background** and **Notify** without changing the core fetch model above.
...
| **M6**    | Polish         | Error/empty states, stale indicator, off-season UX, Lucide icon system (`src/ui/icons.rs`) |

---

## v0.0.0.1.0 MVP scope
...
- Lucide SVG icons for all UI chrome (nav, section headings, controls, boot steps); flags/headshots/logos remain remote images
