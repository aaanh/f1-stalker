...
F1 Stalker v0.1.0 builds on the v0 dashboard (M0&ndash;M6): calendar, pins, championship charts, quali grid, weather, and offline cache. v0.1.0 adds cross-platform distribution, comparison and personalization, background presence with notifications, and onboarding polish.

**Delivery order:** cross-platform distribution → comparison and personalization → background and notifications → onboarding flow.

**Baseline:** [.specs/v0.0.0.md](./v0.0.0.md) is delivered. v0.1.0 does not remove v0 behaviour unless explicitly superseded below.

### Implementation status (current tree)

Shipped on `master` (tag `v0.1.0` and follow-up commits): v0.1.0 dashboard shell, rivalry section, theme presets, sprint grid, notifications, tray/background toggle, simplified first-run overlay, championship narrative, release CI, and macOS packaging scripts.

**In progress (uncommitted):** responsive typography pass and rival panel polish (team logos, layout) remain optional polish.
...
| **M8** Comparison & personalization | Done | Rival avg quali, WCAG AA preset audit, custom theme editor in Settings. |
| **M9** Background & notifications | Done | Tray Open Dashboard / Refresh / Quit; macOS dock hide in background; single-instance focus. |
| **M10** Onboarding | Done | Multi-step first-run wizard (timezone picker, in-flow pin picker). |
...
| OpenF1 | openf1-client (historical) | unchanged; no authenticated endpoints in v0.1.0 |
...
F1 Stalker v0.1.0 uses **historical OpenF1 data only**, same as v0. No OAuth, no paid subscription, no live API calls.
...
Authenticated / real-time OpenF1 is **deferred** to a future release (see [v0.1.0 scope](#v0.1.0-scope) **Out**).
...
### Weekend formats (v0.1.0 updates)
...
| **Sprint in countdown** | N/A | Already in v0 session list; v0.1.0 adds sprint grid row when data exists |
...
| --- | ----- | ---------- | -------- | ----- |
| US0 | user | download a portable build for my CPU architecture (arm64 / x86_64) | I can run the app without installing dependencies | v0.1.0 |
| USF1 | user | have a desktop launcher and app icon | I can open the app readily | v0.1.0 |
| USF2 | user | be guided through first-run setup (timezone, optional pins) | the dashboard works correctly without reading docs | v0.1.0 |
| USF3 | fan | see who leads or has won the Drivers' Championship | I get a clear season narrative at a glance | v0.1.0 |
| USF4 | fan | use a rival mode view with exactly two drivers | I can compare two drivers head-to-head across the season | v0.1.0 |
| US6.1 | user | choose a theme from a default set of constructor livery colours | the app matches my team preference without manual tuning | v0.1.0 |
| US6.2 | user | create a custom theme | the dashboard looks the way I want | v0.1.0 |
| US8 | user | receive desktop notifications about standings changes and sessions | I am reminded without keeping the window open | v0.1.0 |
| US9.1 | user | keep the app running in the menu bar, taskbar, or tray when I close the window | it stays available without occupying my desktop | v0.1.0 |
| US9.2 | user | restore the main window from the background tray/service | I can return to the dashboard quickly | v0.1.0 |
| US9.3 | user | quit the app completely instead of sending it to the background | I can free resources when I am done | v0.1.0 |
| US10 | fan | see the sprint starting grid for pinned drivers on sprint weekends | I know how they start the Saturday sprint | v0.1.0 |
| US11 | user | run F1 Stalker on Windows or Linux | I am not limited to macOS | v0.1.0 |
| US12 | fan | enable live timing and session data during a race weekend | I can follow the session without a 24h delay | post-v0.1.0 |
| US13 | user | sync pins and settings across devices | my setup follows me | post-v0.1.0 |

v0 stories (US1&ndash;US7, USF5) remain satisfied; v0.1.0 extends them where noted in acceptance criteria.
...
- [x] Average quali position in summary
...
- [x] Column-first layout (top-to-bottom, then left-to-right); uses same 1 / 2 / 3 column breakpoints as pinned drivers (`pinned_grid_columns`)
- [x] Latest race mode shows starting grid position per driver
- [x] Position-change arrows (Lucide `chevron-up` / `chevron-down`) beside position: championship compares previous round; latest race compares grid to finish; arrows sized at or above the position numeral (not caption-scale text)
...
- [x] WCAG AA contrast audit for all presets
- [x] Custom theme editor: user picks accent + background + surface; stored in SQLite; export/import optional (`ThemePresetId::Custom` exists as placeholder)
...
- [x] Structural UI (standings position + change arrows, section headings) uses dedicated layout bases &ge; 20px at 1.0 scale; never reuse caption/detail sizes (11&ndash;13px) for primary indicators
...
- [x] Formal Do Not Disturb / spam-on-retry verification
...
- [x] **macOS:** dock icon hidden while in background (`LSUIElement` or equivalent)
- [x] Tray menu: Open Dashboard, **Refresh**, Quit (current: Show, Quit only)
- [x] Quit terminates process
- [x] Single-instance: second launch focuses existing window
...
- [x] Multi-step wizard with editable timezone and in-flow pin picker
...
- [x] Shown regardless of **Compare rivals** chart focus (charts narrow to the pair; banner stays visible)
- [x] Uses same championship snapshots as charts; no extra API surface
- [x] Previous and Current race session cards show podium driver cards (portrait, code, position)
...
### OpenF1 endpoints (additions for v0.1.0)
...
### App lifecycle (v0.1.0 extensions)
...
- [x] Standings table should flow top-bottom then left-right
- [x] Standings should contain starting grid
- [x] Standings should show position diff with Lucide chevron-up / chevron-down icons side by side next to the position number, each either grey or current theme accent color
    - Championship: from immediate previous race to current latest
    - Latest race: from starting grid to final position
    - Arrow icons at **22px** base (20px compact), position numeral at **24px** base (22px compact); both scale with `font_scale`
    - Rank block: fixed-width arrow column + fixed-width **monospace** position column (sized for three glyphs: `DSQ` / `DNF` / `DNS`); position right-aligned so `1`&ndash;`99` line up across cards
- [x] Previous and Current race session cards should show cards of podium drivers

### Typography (dev guidance)

**Do not undersize structural UI.** Early standings work used `layout.text(11)` (~11px) for position-change arrows while the position numeral was 18px. At default scale the chevrons were barely visible and read as broken UI, not data.

| Element | Wrong (anti-pattern) | Correct |
| ------- | -------------------- | ------- |
| Standings change arrows | `layout.text(11)` or `card_detail_size` | `LayoutConfig::standings_arrow_size` (20&ndash;22px base via `scale_px`) |
| Standings position | 18px while arrows smaller | `LayoutConfig::standings_position_column_width` (3 mono glyphs) + `MONO` font, right-aligned |
| Standings grid | Ad-hoc 2 columns only | Reuse `pinned_grid_columns` (1 / 2 / 3 by viewport width) |
| Section icons | Ad-hoc 12px | `section_heading` (16px icon + 15px title) minimum |

Rule: if a glyph carries **position or direction** information, size it at or above adjacent numerals/labels, not at caption/muted footnote scale. Use Lucide SVGs via `icon()`, not Unicode triangles.